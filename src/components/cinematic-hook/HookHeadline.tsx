...
export default function HookHeadline({ elapsed }: { elapsed: number }) {
  const words = [
    { text: 'To See The World', start: 6.0 },
    { text: '&', start: 6.35 },
    { text: 'To Find Each Other', start: 6.5 },
  ];
  const brandOpacity = phaseOpacity(elapsed, 6.85, 7.0, 7.0, 7.6);
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 3, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none', gap: '8px' }}>
      {words.map(({ text, start }) => {
        const opacity = phaseOpacity(elapsed, start, start + 0.2, 7.0, 7.6);
        const translateY = opacity < 1 ? (1 - opacity) * 12 : 0;
        return (
      ...
      <div style={{ fontFamily: '"JetBrains Mono", monospace', fontSize: '11px', color: '#8B7355', letterSpacing: '0.25em', textTransform: 'uppercase', opacity: brandOpacity, marginTop: '16px' }}>
        TheEndProductions
      </div>
    </div>
  );
