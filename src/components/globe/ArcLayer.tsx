...
export default function ArcLayer({ pins }: ArcLayerProps) {
  ...
  return (
    <group>
      {arcs.map(({ from, to }) => {
        const start = new THREE.Vector3(...latLngToVector3(from.latitude, from.longitude, 0.005));
        const end = new THREE.Vector3(...latLngToVector3(to.latitude, to.longitude, 0.005));
        const mid = start.clone().add(end).multiplyScalar(0.5);
        const dist = greatCircleDistance(from.latitude, from.longitude, to.latitude, to.longitude);
        mid.normalize().multiplyScalar(1 + dist * 0.15);
        ...
        return (
          <Line key={`${from.id}-${to.id}`} points={points} color={color} lineWidth={1.2} transparent opacity={0.5} />
        );
      })}
    </group>
  );
