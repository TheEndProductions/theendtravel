    ...
    <>
      <div>
        ...
        <section style={{ position: 'relative', padding: '200px 24px', overflow: 'hidden' }}>
          <video src="/waterfall_bali_web.mp4" muted loop playsInline autoPlay style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.5, zIndex: 0 }} />
          ...
          <div style={{ position: 'relative', zIndex: 2, maxWidth: '1088px', margin: '0 auto', textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center' }}>
            ...
            <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '16px', color: '#D4CFC7', lineHeight: 1.7, margin: 0, maxWidth: '800px' }}>Every film, every product & every project begins with a simple question: Who are we? The End Productions exists at the intersection of exploration, storytelling & human connection.</p>
          </div>
        </section>

        {/* 3. Endless Backpack */}
        <section id="endless" style={{ position: 'relative', padding: '100px 24px', overflow: 'hidden' }}>
          <img src="/endless-section-bg.jpg" alt="" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', opacity: 0.5 }} />
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, #0A0A0A 0%, rgba(10,10,10,0.2) 20%, rgba(10,10,10,0.2) 80%, #0A0A0A 100%)' }} />
        ...
        <Section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '24px' }}>
            {['I Once Imagined', 'Altruism', 'SALT'].map((title, i) => (
        ...
        <section style={{ position: 'relative', width: '100vw', minHeight: '80vh', overflow: 'hidden', marginLeft: 'calc(-50vw + 50%)', marginRight: 'calc(-50vw + 50%)' }}>
          <div style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
            <GlobePreview />
          </div>
          <div style={{ position: 'absolute', inset: 0, zIndex: 1, background: 'linear-gradient(to right, rgba(10,10,10,0.75) 0%, rgba(10,10,10,0.3) 50%, transparent 100%)' }} />
          <div style={{ position: 'relative', zIndex: 2, display: 'flex', alignItems: 'center', minHeight: '80vh', padding: '60px 40px' }}>
        ...
        <Section>
          <div style={{ maxWidth: '640px', margin: '0 auto', textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center' }}>
            ...
            <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '16px', color: '#D4CFC7', lineHeight: 1.7, margin: 0 }}>Our rhyme, our reason, our muse, however you want to refer to it, we believe in giving back, every step of the way. At The End, we extend an open hand to all, inviting you to join our community. Travel should leave places better than we found them.</p>
