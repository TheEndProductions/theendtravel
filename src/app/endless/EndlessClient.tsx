...
import BagAmbientBackground from '@/components/endless/BagAmbientBackground';

const Section = ({ children, bg }: { children: React.ReactNode; bg?: string }) => (
  <section style={{ padding: '40px 24px 200px', background: bg || 'transparent' }}>
  ...
  return (
    <div style={{ background: '#0A0A0A', minHeight: '100vh' }}>
      ...
      <section style={{ position: 'relative', width: '100%', minHeight: '70vh', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, minHeight: '70vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '20px', textAlign: 'center', padding: '40px 24px 200px' }}>
      ...
      {/* Pieces — with cross-fading ambient background */}
      <section style={{ position: 'relative', padding: '40px 24px 200px', overflow: 'hidden' }}>
        <BagAmbientBackground activeId={activeId} />
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '1000px', margin: '0 auto' }}>
          ...
          <PieceSelector activeId={activeId} setActiveId={setActiveId} />
        </div>
      </section>
      ...
      <Section>
        ...
        <div style={{ display: 'flex', gap: '32px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: '0 1 320px', padding: '28px', borderRadius: '12px', border: '1px solid rgba(196,83,10,0.3)', background: 'linear-gradient(135deg, rgba(196,83,10,0.08) 0%, rgba(10,10,10,0.6) 100%)', display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center', textAlign: 'center' }}>
            ...
            <div style={{ width: '100%', height: '1px', background: 'rgba(245,242,237,0.08)' }} />
            ...
            <p style={{ fontFamily: '"DM Sans", sans-serif', fontSize: '14px', color: '#D4CFC7', lineHeight: 1.6, margin: 0 }}>Join the list and get 15% off your entire order when we launch. Be first in line.</p>
          </div>
        </div>
      </Section>
